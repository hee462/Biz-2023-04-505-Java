 ...
 * 일반적으로 컴퓨터를 이용하는 정보처리는 다수의 데이터를 효과적으로 처리하는 목적이 있다.
 * 다수의 데이터를 빠른시간에 효율적으로 처리하고, 답을 구하는 것이 정보처리이다.
 * 
 * 이 때 변수를 선언하는데, 한번에 다수의 변수를 선언하고, 사용하는 방법으로 
 * 배열(Array) 라는 것을 시작한다.
 * 
 * 
 */
public class ArrayA {
	...
	public static void main(String[] args) {
		//국어 점수를 지정할 intKor 변수를 10개 생성하기.
		/*  
		 * int[] :  정수형 데이터를 저장할 기억장소 공간이 여러개 필요하다. 라는 선언
		 *   => 배열 선언 키워드
		 *   
		 *    저장소의 이름은 intKor 로 하겠다.
		 *    ? 몇개가 필요한데 .. new int[10] : 10개의 데이터 저장공간이 필요하다.
		 *    
		 */
		int[] inkor = new int[10];
