package com.hee462.system.exec;

public class NumbersA {
	public static void main(String[] args) {
		// 문자열형 숫자
		// 덧셈(연결) 연산만 가능 다른 사칙연산은 불가능
		// 숫자로 보이지만 실제 데이터는 문자열이다.
		String strNum1 = "30";
		String strNum2 = "30.0";
		String strNum3 = "30 + 40 ";
		
		
		// 문자열형 숫자를 4칙 연산이 가능한 숫자로 변환하기
		int num1 = Integer.valueOf(strNum1);
		float num2 = Float.valueOf(strNum2);
		/*
		 * 문자열형 숫자를 실제 4칙 연산이 가능한 숫자로 변환하기 위해서는
		 * 숫자형 문자열 외에 어떠한 문자열도 포함 할 수 없다.
		 */
		System.out.println(Integer.valueOf("30"));
//	실	System.out.println(Integer.valueOf("  30"));
//	행	System.out.println(Integer.valueOf("30  "));
//	안	System.out.println(Integer.valueOf("A30"));
//	됨	System.out.println(Integer.valueOf("30A"));
//		int num3 = Integer.valueOf(strNum3);
		
		System.out.println(num1+num2);
		System.out.println(num2+num1);
		
		
		
		
	}
}
