...
public class DateExecA {
	 ...
	 *  자바에서는 1.8 기준으로 날짜와 시간을 처리하는 방식이 차이가 있다.
	 *  또한 프로그래밍 언어, 시스템, 국가, 소프트웨어 마다 처리하는 방식이 차이가 있다
	 *  
	 *  그래서 날짜와 관련된 이슈를 피하기 위해서 여러가지 기법이 사용되는데
	 *  그중 많이 사용하는 방법중 하나가 날짜 type 의 데이터를 문자열로 변환하여 사용하는 방법이다.
	 */
	public static void main(String[] args) {
		//1.7 버전 이하에서 사용하는 날짜 type 의 객체
		Date date = new Date(System.currentTimeMillis());
		System.out.println(date.toString());
		
		SimpleDateFormat dateFormat = new SimpleDateFormat("YYYY-MM-dd");
		SimpleDateFormat timeFormat = new SimpleDateFormat("HH:mm:ss");
		// 날짜 type을 문자열로 변환하는 코드
