...
public class NumbersB {
	...
	public static void main(String[] args) {
		...
		// 문자열형 숫자를 4칙 연산이 가능한 정수형 숫자로 변환
		int intNum1 = Integer.parseInt(strNum1);  
		Integer intNum2 = Integer.valueOf(strNum2);  // 전체적으로 type 변환 되기 때문에 parse형은 추천하지 않는다.
		int  intNum3 = Integer.valueOf(strNum1);
		
		/*
		 * 문자열형 숫자를 4칙 연산이 되는 숫자 데이터를 변환시키는 방법에는 2가지 method가 있다.
		 ...
		 * 		일부에서는 성능상 Issue 가 있다고 하지만 지금은 크게 문제 되지 않는다.
		 * 
		 * 문자열형 숫자를 실제 숫자로 바꿀때는 Type.valeOf() method를 사용해도 큰 문제가 없다.
		 */
